...
export class CommnetService {
	...
	addCommentService = async (commentData: Omit<CommentEntity, "user_id">, user_id: string): Promise<void> => {
		const newComment = commentFactory(commentData, user_id);
		await this._commentReposiotory.addComment(newComment);
	};

	editCommentService = async (commentChanges: Omit<CommentEntity, "post_id" | "user_id" | "created_at">, user_id: string): Promise<void> => {
		await this._commentReposiotory.editComment(commentChanges, user_id);
	};

	deleteCommentService = async (comment_id: string, user_id: string): Promise<void> => {
		const ids = {
			id: comment_id,
			user_id: user_id,
		};
	...
	loadCommentsService = async (post_id: string, offset: number): Promise<CommentEntity[]> => {
