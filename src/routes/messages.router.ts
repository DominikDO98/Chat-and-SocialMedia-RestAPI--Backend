...
import { checkMessageAsDeliveredController, deleteMessageController, sendMessageController } from "../controllers/message.controller";
...
MessageRouter
	...
	.get("/loadMessages", async (req: Request, res: Response, next: NextFunction) => {
		validateReq(req, ["id"]);
		await sendMessageController(req, res, next);
	})
