...
export const createChatRepo = async (contact_id: string, chatData: ChatEntity): Promise<void> => {
...
export const createGroupChatRepo = async (participantsIds: string[], chatData: ChatEntity): Promise<void> => {
	const client = await pool.connect();
	try {
		await client.query("BEGIN");
		await client.query("INSERT INTO chats (id, is_group, name) VALUES ($1, true, $2)", [chatData.id, chatData.name]);
		await addUsersLoop(participantsIds, client, chatData.id);
		await client.query("COMMIT");
	} catch (err) {
		console.log(err);
		client.query("ROLLBACK");
...
export const changeChatNameRepo = async (chat_id: string, newName: string): Promise<void> => {
	await pool.query("UPDATE chats SET name = $1 WHERE id = $2", [newName, chat_id]);
};

export const loadPrivateChatsRepo = async (user_id: string): Promise<PrivateChatDataEntity[]> => {
...
export const loadGoupChatsRepo = async (user_id: string): Promise<GroupChatDataEnitity[]> => {
	const { rows } = await pool.query(
...
export const deleteGroupChatRepo = async (chat_id: string): Promise<void> => {
	const client = await pool.connect();
	try {
		await client.query("BEGIN");
...
export const deleteUserFromGroupRepo = async (user_id: string, chat_id: string): Promise<void> => {
