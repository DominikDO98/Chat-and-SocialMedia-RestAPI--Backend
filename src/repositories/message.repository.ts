...
export class MessageRepository {
	...
	static sendMessageRepo = async (message: MessageEntity): Promise<void> => {
	...
	static loadMessagesRepo = async (chat_id: string, offset: number): Promise<MessageEntity[]> => {
	...
	static deleteMessageRepo = async (mess_id: string): Promise<void> => {
		await pool.query("DELETE FROM messages WHERE id = $1", [mess_id]);
	};

	static checkMessagesAsDeliveredRepo = async (chatId: string): Promise<void> => {
