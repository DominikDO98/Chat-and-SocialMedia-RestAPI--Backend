...
export class CommentRepository {
	...
	static addCommentRepo = async (commentData: CommentEntity): Promise<void> => {
	...
	static editCommentRepo = async (commentChanges: Omit<CommentEntity, "created_at" | "post_id" | "user_id">, user_id: string): Promise<void> => {
	...
	static deleteCommentRepo = async (ids: Pick<CommentEntity, "id" | "user_id">): Promise<void> => {
		await pool.query("DELETE FROM comments WHERE id = $1 AND user_id = $2", [ids.id, ids.user_id]);
	};

	static loadCommentsRepo = async (post_id: string, offset: number): Promise<CommentEntity[]> => {
