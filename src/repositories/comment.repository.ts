...
export class CommentRepository {
	...
	static addComment = async (commentData: IComment): Promise<void> => {
	...
	static editComment = async (commentChanges: Omit<IComment, "created_at" | "post_id" | "user_id">, user_id: string): Promise<void> => {
	...
	static deleteComment = async (ids: Pick<IComment, "id" | "user_id">): Promise<void> => {
		await pool.query("DELETE FROM comments WHERE id = $1 AND user_id = $2", [ids.id, ids.user_id]);
	};

	static loadComments = async (post_id: string, offset: number): Promise<IComment[]> => {
