...
export class CommentRepository {
	...
	static addComment = async (commentData: CommentEntity): Promise<void> => {
	...
	static editComment = async (commentChanges: Omit<CommentEntity, "created_at" | "post_id" | "user_id">, user_id: string): Promise<void> => {
	...
	static deleteComment = async (ids: Pick<CommentEntity, "id" | "user_id">): Promise<void> => {
		await pool.query("DELETE FROM comments WHERE id = $1 AND user_id = $2", [ids.id, ids.user_id]);
	};

	static loadComments = async (post_id: string, offset: number): Promise<CommentEntity[]> => {
